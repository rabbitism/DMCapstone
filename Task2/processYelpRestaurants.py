...
def main(save_sample, save_categories):
    ...
    if save_sample:  
        ...
        with open("positive_reviews.txt", 'wb') as f:
            for i in range(len(sample_ratings)):
                if(sample_ratings[i]>'3'):
                    f.write((my_sample_v2[i]+'\n').encode('ascii', 'ignore'))
        with open("negative_reviews.txt", 'wb') as f:
            for i in range(len(sample_ratings)):
                if(sample_ratings[i]<'3'):
                    f.write((my_sample_v2[i]+'\n').encode('ascii', 'ignore'))
...
def sim_matrix():
    ...
    for doc_a in labeled_topics:
        ...
        clustered_matrix.append(sim_vecs)
    
    with open( 'clustered_sim_matrix.csv', 'w') as f:
        for i_list in clustered_matrix:
            s = ""
            #print(i_list)
            my_max = max(i_list)
            my_max = 1 if my_max==0 else my_max
    ...
    with open('labeled_cuisine.txt', 'w') as f:
        f.write( "\n".join(labeled_cuisines))
